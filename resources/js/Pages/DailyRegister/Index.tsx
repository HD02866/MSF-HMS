...
// ── Add Entry Modal ────────────────────────────────────────────────────────────
function AddEntryModal({ types, onClose }: { types: RegisterType; onClose: () => void }) {
    const form = useForm({
        patient_id: '',
        register_type: 'family',
        record_date: new Date().toISOString().split('T')[0],
        department_name: '',
        // Auto-fill display fields (not submitted)
    });
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            <Input
                                ...
                                placeholder="Enter card number e.g. 97266-0"
                                className="flex-1"
                            />
...
// ── Edit Modal ─────────────────────────────────────────────────────────────────
function EditModal({ entry, types, onClose }: { entry: RegisterEntry; types: RegisterType; onClose: () => void }) {
    const form = useForm({
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
                ...
                <p className="text-sm text-gray-600 mb-6">
                    Remove register entry for{' '}
                    <span className="font-semibold">{entry.patient.full_name}</span>?
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function DailyRegisterIndex({ registers, summary, types, filters, canManage }: Props) {
    ...
    // Filter form state (local, submitted on change)
    ...
    return (
        <AppLayout title="Daily Register">
            ...
            {/* Modals */}
            {showAdd && <AddEntryModal types={types} onClose={() => setShowAdd(false)} />}
            {editEntry && <EditModal entry={editEntry} types={types} onClose={() => setEditEntry(null)} />}
            {deleteEntry && <DeleteModal entry={deleteEntry} onClose={() => setDeleteEntry(null)} />}
            ...
            <div className="bg-white rounded-xl border shadow-sm p-4 mb-5">
                <div className="flex flex-wrap gap-3 items-end">
                    ...
                    <div className="flex-1 min-w-[140px]">
                        ...
                        <Input
                            type="date"
                            value={localFilters.record_date}
                    ...
                    <div className="flex-1 min-w-[160px]">
                        ...
                        <Select
                            value={localFilters.register_type}
                    ...
                    <div className="flex-1 min-w-[160px]">
                        ...
                        <Input
                            value={localFilters.search_name}
            ...
            <div className="flex flex-wrap gap-2 mb-3">
                {Object.entries(types).map(([key, label]) => (
                    <span
                        key={key}
                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${TYPE_BADGE_COLORS[key] ?? 'bg-gray-100 text-gray-700'}`}
                    >
                        {label}
                    </span>
                ))}
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {registers.data.map((entry, index) => (
                                    <tr
                                        key={entry.id}
                                        className={`border-t ${ROW_COLORS[entry.register_type] ?? ''}`}
                                    >
                                        <td className="px-4 py-3 text-gray-400 text-xs">
                                            {(registers.from ?? 1) + index}
                                        </td>
